...
\begin{definition}[Discrete-time Markov chains]
		A discrete-time Markov chain (DTMC) consists of a \textit{set of states} $S = \{1, \dots, n\}$ and a family of random variables $(S_t)_{t \in \mathbb{N}}$ over $S$, and which respects the Markov property:
				$$S_{t+1} \ind S_0, \dots, S_{t-1} \mid S_t$$
which means:
...
\begin{definition}[Markov Reward Process]
		...
		\begin{itemize}
				...
				\item We replace the Markov property by this stronger version:
						$$S_{t+1} \ind S_0, R_1, S_1, R_2, \dots, S_{t-1}, R_t \mid S_t$$
						$$\mathbb{E}[R_{t+1} \mid S_0 = s_0, \dots, S_t = s_t, R_1 = r_1, \dots, R_t = r_t] = \mathbb{E}[R_{t+1} \mid S_t = s_t]$$
				\item We assume rewards are homogeneous, therefore the new homogeneousness condition becomes:
						$$\mathbb{P}(S_{t+2} = s' \mid S_{t+1} = s) = \mathbb{P}(S_{t+1} = s' \mid S_t = s)$$
		...
		\end{itemize}
		Some authors make one more assumption about rewards to make the homogeneousness condition of rewards even stronger:
...
\begin{definition}
		...
		\begin{itemize}
				...
				\item We replace the Markov property by this stronger version:
						$$S_{t+1} \ind S_0, A_0, R_1, S_1, A_1, R_2, \dots, S_{t-1}, A_{t-1}, R_t \mid S_t$$
						$$\mathbb{E}[R_{t+1} \mid S_0 = s_0, \dots, S_t = s_t, A_0 = a_0, \dots, A_t = a_t, R_1 = r_1, \dots, R_t = r_t] = \mathbb{E}[R_{t+1} \mid S_t = s_t, A_t = a_t]$$
				\item We replace the homogeneousness condition by this stronger version:
						$$\mathbb{P}(S_{t+2} = s' \mid S_{t+1} = s, A_{t+1} = a) = \mathbb{P}(S_{t+1} = s' \mid S_t = s, A_t = a)$$
